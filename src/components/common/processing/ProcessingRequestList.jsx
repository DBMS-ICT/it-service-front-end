...
import { useGetOldRequestQuery } from '../../../app/api/request';
...
function ProcessingRequestList() {
  ...
  const {
    ...
    isError: getIsError,
  } = useGetOldRequestQuery(page);

  return (
    <div>
      {!getIsError && getISLaoding ? (
        <Loader />
      ) : (
          ...
          <tbody className="overflow-hidden">
            {getData?.data.map((item, index) => (
              ...
              >
                ...
                <td className="border-b text-center border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">{item?.problem}</p>
                </td>
                <td
                ...
                >
                  <div className="flex items-center justify-center space-x-3.5">
                    <Link to={`/processing/request/edit/${item?._id}`}>
                      <IoEye
                        size={27}
...
export default ProcessingRequestList;
